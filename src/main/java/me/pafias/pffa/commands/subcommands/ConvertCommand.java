...
public class ConvertCommand extends BaseFFACommand {
    ...
    public void execute(String mainCommand, CommandSender sender, String[] args) {
        if (!(sender instanceof ConsoleCommandSender)) {
            sender.sendMessage(LCC.t("&cFor security reasons, only console may execute this command."));
            return;
        }
        if (args.length < 1) {
            sender.sendMessage(LCC.t("&c/ffa convert " + getArgs()));
            return;
        }
        final File playerdataDirectory = new File(args[1]);
        if (!playerdataDirectory.exists() || !playerdataDirectory.isDirectory()) {
            sender.sendMessage(LCC.t("&cThe specified folder does not exist or is not a directory."));
            return;
        }
        if (playerdataDirectory.listFiles().length == 0) {
            sender.sendMessage(LCC.t("&cThe specified folder is empty."));
            return;
        }
        if (!confirm.containsKey(sender)) {
            confirm.put(sender, true);
            sender.sendMessage("");
            sender.sendMessage(LCC.t("&eThis will kick everyone from the server and convert all the player data in the specified folder to the new format set in the config (file or database)."));
            sender.sendMessage(LCC.t("&eIf you are sure about this and want to proceed, run the command again."));
            sender.sendMessage("");
            Tasks.runLaterAsync(30 * 20, () -> {
                confirm.remove(sender);
                sender.sendMessage(LCC.t("&cConversion confirmation expired."));
            });
            return;
        } else {
            confirm.remove(sender);
        ...
        if (sender instanceof Player)
            ((Player) sender).kickPlayer(LCC.t("&cConverting player data... Please rejoin after a few seconds."));
        plugin.getServer().getOnlinePlayers().forEach(player -> {
            player.kickPlayer(LCC.t("&cPlease rejoin in a few seconds."));
        });
        int i = 0;
        ...
        sender.sendMessage(LCC.t("&aConverted " + i + " player data files."));
    }
