    ...
    public void execute(String mainCommand, CommandSender sender, String[] args) {
        ...
        Tasks.runAsync(() -> {
            if (cachedLeaderboards.containsKey(statistic)) { // Already cached
                ...
                if (Duration.between(lastFetch, Instant.now()).toMillis() < 3000) { // Too little time has passed, use cache
            ...
            for (UserData userData : userDataList) {
                ...
                switch (statistic) {
                    case KILLS:
                        value = userData.getFfaData().getKills();
                        break;
                    case DEATHS:
                        value = userData.getFfaData().getDeaths();
                        break;
                    case KILLSTREAK:
                        value = userData.getFfaData().getKillstreak();
                        break;
                }
